...
import pytest
from ezfalcon.dynamics.forces.self_gravity.falcON import _falcON_gravity
...
def test_newtons_third_law():
    ...
    acc = _falcON_gravity(pos, mass, eps=0.0, theta=0.0, kernel=1, return_potential=False)
...
def test_two_body_acceleration():
    ...
    acc = _falcON_gravity(pos, mass, eps=0.0, theta=0.0, kernel=0, return_potential=False)
...
def test_potential_against_direct():
    ...
    pos = np.random.normal(size=(2,3))
    mass = 10**np.random.normal(loc = 10, scale=1, size=(2,))
    _, pot = _falcON_gravity(pos, mass, eps=0.0, theta=0.0, kernel=1, return_potential=True)
...
def test_zero_acceleration_for_single_particle():
    ...
    acc, pot = _falcON_gravity(pos, mass, eps=0.0, theta=0.0, kernel=0, return_potential=True)
...
def test_acc_and_pot_shapes():
    '''
    Test that the falcON method returns acceleration and potential arrays of the correct shape.
    '''
    pos = np.random.normal(size=(3,3))
    mass = 10**np.random.normal(loc = 10, scale=1, size=(3,))
    acc, pot = _falcON_gravity(pos, mass, eps=0.0, theta=0.1, kernel=1, return_potential=True)
...
def test_return_potential_true_returns_tuple():
    ...
    pos = np.random.normal(size=(2,3))
    mass = 10**np.random.normal(loc = 10, scale=1, size=(2,))
    out = _falcON_gravity(pos, mass, eps=0.0, theta=0.1, kernel=1, return_potential=True)
